# -*- coding: utf-8 -*-
"""
Created on Mon Nov 13 09:37:02 2023
...
if len(sys.argv) == 1:
    ...
    parser.add_option("-m", "--month", dest="month", action="store", type="string",
                      help="month (int): Select month (format:2 numbers),if None=All year processed",default=None)
    parser.add_option("-d", "--download_path", dest="dpath", action="store", type="string",
                      help="download_path (str): Specify the path to store OutputCSV", default='.')
    parser.add_option("-s", "--service", dest="svc", action="store", type="string",
                      help="service (str): For now, only 'peps', 'theia','landsat' and 'earth_search_gcs' is supported", default='peps')
...
# svc='earth_search_gcs' #Service used for research 'landsat','peps','theia','earth_search_gcs'(Google Earth engine equivalent)
# dpath=r'C:\Travail\Script\Listing' #Where to create csv with output data (or where is already stored output csv)
# startyear='2015'       #year to search
# tile='48PWQ'
# month='01'
###########################################################################

Year=options.year.replace('\r','') # Remove linebreak because I dont know how to do it in .sh :)
...
if not os.path.exists(os.path.join(format(os.getcwd()),Year)): #create year file if it doesn't already exist
    os.mkdir(Year)
os.chdir(os.path.join(format(os.getcwd()),Year))

if not os.path.exists(os.path.join(format(os.getcwd()),Tiles)):
    os.mkdir(Tiles)
os.chdir(os.path.join(format(os.getcwd()),Tiles))



def EODAG_search(year=None,month=None,download_path=None,service='peps',productTypes='S2_MSI_L1C',tileIdentifiers=None,**kwargs):
    '''Run EODAG with specified parameters
    Args:
        year (str): Select start year (DUH...).
    ...
    if year==None or download_path==None or tileIdentifiers==None:
    ...
    starts='{}-{}-01'.format(year,month)
    ends='{}-{}-{}'.format(year,month,str(calendar.monthrange(int(year), int(month))[1])) 
    if service=='peps':
    ...
    Online=online_search_results
    Offline=offline_search_results
...
if options.month==None:
    for j in range (1,13,1):
        ...
        online,offline=EODAG_search(year=Year,month=month,download_path=options.dpath,service=services,productTypes=options.product_type,tileIdentifiers=Tiles)
        
        Listd=[]
        Listn=[]
        
        for t in online:
            Listd.append(re.search(r'\d{8}', str(t)).group())
            Listn.append(re.search(r'\d{2}[A-Z]{3}', str(t)).group())
        d = {'Date': Listd,'Tile': Listn}
        dft=pd.DataFrame(data=d)
        date=pd.to_datetime(dft.iloc[:,0],format='%Y%m%d',yearfirst=True)
        data={'Tile':dft.iloc[:,1],'Date':date,'Online':[1]*len(dft.iloc[:,1])}
        ...
        dft=pd.DataFrame(data=d)
        date=pd.to_datetime(dft.iloc[:,0],format='%Y%m%d',yearfirst=True)
        data={'Tile':dft.iloc[:,1],'Date':date,'Online':[0]*len(dft.iloc[:,1])}
        dff=pd.DataFrame(data)
        dff=dff.set_index('Date')
        allt=pd.concat([dff,dfo])
        
        allt.to_csv(path_or_buf=os.path.join(os.getcwd(),'{}_{}_{}_data.csv'.format(services,Tiles,month)),sep=';',date_format='%Y%m%d')
else:
    month=options.month
    online,offline=EODAG_search(year=Year,month=month,download_path=options.dpath,service=services,productTypes=options.product_type,tileIdentifiers=Tiles)
