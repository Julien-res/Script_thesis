...
if __name__=="__main__":
    ...
    if options.d_path==None:
        if not os.path.isdir('eodag_workspace'):
            os.mkdir('eodag_workspace')
            os.chdir(os.path.join(os.getcwd(),'eodag_workspace'))
            dpath=os.getcwd()
    else :
        dpath=options.d_path
    
    yaml_path=create_yaml(credential=options.credential,service=services,dpath=dpath)
    src_path=os.path.join(dpath,'Search_results')
    ...
    Online,Offline=EODAG_search(download_path=dpath,service=services,productTypes='S2_MSI_L1C',
