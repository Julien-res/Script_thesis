...
# Newmask  ==============
fig, ax = plt.subplots()
TEST = np.where(PDATA[0]==2, 0, PDATA[0])
TEST = np.where(TEST!=0, 1, TEST)
TEST=TEST-WDATA[0]
TEST = np.where(TEST!=1, np.nan, TEST)
heatmap = ax.imshow(TEST,cmap='Greys',vmin=0,vmax=1)
ax.tick_params(
    axis='both',
    which='both',
    bottom=False,
    ...
    labelleft=False)
plt.tight_layout()
plt.savefig(fname=os.path.join(os.getcwd(),'Newmask.png'),format='png',dpi=1000)