...
def filter_srf_file(input_path, output_path, threshold=0.01):
    # Lire le fichier CSV
    df = pd.read_csv(input_path)

    # Remplacer les valeurs inférieures au seuil par 0
    df = df.applymap(lambda x: x if isinstance(x, (int, float)) and x >= threshold else 0)

    # Sauvegarder en CSV
    df.to_csv(output_path, index=False)

# Exemple d'utilisation
input_file = "/mnt/c/Travail/Script/Script_thesis/5_SPM_POC/Determination_Best_algo/SRF/"
output_file = "/mnt/c/Travail/Script/Script_thesis/7_Random_figures/SRF_band/All_bands/filtered/"

filter_srf_file(input_file + 'SRF_MODIS.csv', output_file+'SRF_MODIS_filt.csv', threshold=0.01)

fig = plot_srf(output_file, 'MODIS_filt', 12)
output_path = os.path.join(output_file, f"MODISfiltered_all_bands.png")
fig.savefig(output_path)
plt.close(fig)

fig = plot_srf(input_file, 'MODIS', 12)
output_path = os.path.join(output_file, f"MODIS_all_bands.png")
fig.savefig(output_path)
plt.close(fig)