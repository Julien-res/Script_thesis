...
OUTPUT='/work/users/cverpoorter/VolTransMESKONG/Data/S2_PEPS/S2_PEPS_OWT/'
    ...
    for month in monthdic : #For all month
        ...
        for pathdt in monthdic[month] : # For all images in those month
            ...
            if not os.path.exists(OUTPUT+os.path.basename(pathdt)+'.tif'):
                ...
                if np.isnan(np.vectorize(testnan)(bitmask)).all():
                    ...
                    for b in range(0,len(DATA)):
                        ...
                        if not np.isnan(np.where(bitmask,DATA[b],np.nan)).all(): #test doesn't work? => no nan but 0, test if all 0
                            ...
                            CONTROLE=1
                    print('Output '+os.path.basename(pathdt)+'.tif')
                    if CONTROLE==0:
                        try :
                            OUT = Chl_CONNECT(tmp,method='logreg',sensor='MSI',logRrsClassif=False,pTransform=False).Class
                        except:
                            print("Can't process the code properly. Possible error with multiple images the same day. ")
                            with open('OUTPUT_BAD_IMG.txt', 'a') as the_file:
                                the_file.write(pathdt+'\n')
                            CONTROLE=2
                        if CONTROLE==0:
                            driver = gdal.GetDriverByName("GTiff")
                            outdata = driver.Create(OUTPUT+os.path.basename(pathdt)+'.tif', 5490, 5490, 1, gdal.GDT_UInt16) #UInt16
                ...
                print('======Image already exist in file======')
            t += 1
        print (t)
