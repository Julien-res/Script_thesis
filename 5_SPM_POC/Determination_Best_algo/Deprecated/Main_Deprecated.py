...
for band in ['B3', 'B5', 'B7']:
    ...
    common_wl = np.linspace(int(wl_s2a[band].values[0])-2, int(wl_s2a[band].values[-1])+2, 1000)

    interp_meris[band] = interp1d(wl_meris[band], srf_meris[band], kind='linear', bounds_error=False, fill_value=0)
    interp_s2a[band] = interp1d(wl_s2a[band], srf_s2a[band], kind='linear', bounds_error=False, fill_value=0)

    srf_meris_resampled = interp_meris[band](common_wl)
    srf_s2a_resampled = interp_s2a[band](common_wl)
    plt.plot(common_wl, srf_meris_resampled, label="MERIS SRF")
    
    weights = srf_meris_resampled / (srf_s2a_resampled + 1e-6)
    s2_fitted = weights * srf_s2a_resampled

    plt.plot(common_wl, s2_fitted, label="Sentinel-2 Fitted", linestyle="dashed")
    plt.legend()
    plt.show()
