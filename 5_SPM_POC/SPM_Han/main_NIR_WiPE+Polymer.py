...
def calculate_spm(rrs_665,rrs753):
    """
    ...
    A_rho_H, C_rho_H = 2220.066, 0.4029
    rho_w_753_clipped = np.clip(rho_w_753[high_turbidity], None, C_rho_H * 0.99)  # Clip to avoid overflow
    spm[high_turbidity] = (A_rho_H * rho_w_753_clipped) / (1 - rho_w_753_clipped / C_rho_H)
    ...
    if np.any(transition_zone):
        ...
        if rrs_753[transition_zone] < 0.03:
            w_H = 0
        elif rrs_753[transition_zone] > 0.04:
            w_H = 1
        else:
            w_H = np.log10(rrs_753[transition_zone]) - np.log10(0.03)
        spm_L = (A_rho_L * rho_w_665[transition_zone]) / (1 - rho_w_665[transition_zone] / C_rho_L)
        spm_H = (A_rho_H * rho_w_753[transition_zone]) / (1 - rho_w_753[transition_zone] / C_rho_H)
...
def process_data(band1,band2, mask, func):
    band1_masked = np.where(mask == 1, band1, np.nan)  # Appliquer le masque
    band2_masked = np.where(mask == 1, band2, np.nan)  # Appliquer le masque
    valid_pixels_B1 = ~np.isnan(band1_masked)  # Identifier les pixels valides
    valid_pixels_B2 = ~np.isnan(band2_masked)  # Identifier les pixels valides
    valid_pixels = valid_pixels_B1 & valid_pixels_B2
    processed_data = np.full_like(band1_masked, np.nan)  # Initialiser le tableau de sortie avec NaN
    # Appliquer la fonction de manière vectorisée uniquement sur les pixels valides
    processed_data[valid_pixels] = func(band1_masked[valid_pixels],band2_masked[valid_pixels])
...
if __name__ == "__main__":
    ...
    for polymer_path in tqdm(polymer_files, desc="Traitement des fichiers POLYMER"):
        ...
        if os.path.exists(output_file):
            ...
            continue
        # Charger les données
        Rrs665 = load_polymer_data(polymer_path, variable_name="Rw665")
        Rrs753 = load_polymer_data(polymer_path, variable_name="Rw753")
        ...
        target_shape = (Rrs665.shape[0], Rrs665.shape[1])
        ...
        # Traiter les données
        processed_data = process_data(Rrs665 ,Rrs753 , water_mask_resampled, calculate_spm)
